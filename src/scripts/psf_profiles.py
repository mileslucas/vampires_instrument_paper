...
nb_filters = ("Ha-Cont", "Halpha", "SII", "SII-Cont")
mbi_filters = ("F610", "F670", "F720", "F760")
...
fig, axes = pro.subplots(
    nrows=2, width="3.5in", height="3.25in", hspace=0.4, sharey=0, hratios=(0.7, 0.3)
)
...
for ax in axes:
    ax.axvline(7.7 * ave_lamd, c="0.5", ls=":", lw=0.7, zorder=0)
    ax.axvline(46.6 / 2 * ave_lamd, c="0.5", ls="-", lw=0.7, zorder=0)
    ax.axvline(64 / 2 * ave_lamd, c="0.5", ls="--", lw=0.7, zorder=0)

axes[0].text(
    17 / 2 * ave_lamd + 6e-3,
    1,
    "AO188 control radius",
    c="0.5",
    rotation=-90,
    fontsize=6,
    va="top",
...
axes[0].text(
    46.6 / 2 * ave_lamd + 6e-3,
    1,
    "SCExAO control radius",
    c="0.5",
    rotation=-90,
    fontsize=6,
    va="top",
...
axes[1].format(ylabel="encircled energy", ylim=(0, None))
# save output
