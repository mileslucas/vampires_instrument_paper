...
def plot_nrm_mask(mask: NRMMask, ax, name):
    for coord in mask.coords:
        patch = patches.Circle(
            coord, radius=mask.hole_rad, color="w", fill=True, zorder=350
        )
        ax.add_patch(patch)
    ax.text(0.03, 0.96, name, ha="left", va="top", c="w", fontsize=8, transform="axes")
    return ax
...
data_dict = {
    ...
    "SAM 18": paths.data
    / "nrm_masks"
    / "vampires_g18_reflective_ThickSpiders_NUDGED.txt",
    "SAM Ann": paths.data / "nrm_masks" / "annulus_reflectivebench_ThickSpiders.txt",
}

fig, axes = pro.subplots(
    nrows=2, ncols=4, width="7in", wspace=0.25, sharex=0, sharey=0, spanx=True
)
...
for i, key in tqdm.tqdm(enumerate(data_dict), total=len(data_dict)):
    ...
    outer_circ = patches.Circle(
        (0, 0), radius=7.95 / 2, ec="w", fc="0.4", lw=1, fill=True, zorder=300
    )
    axes[0, i].add_patch(outer_circ)
    inner_circ = patches.Circle(
        (0, 0), radius=2.337 / 2, ec="w", fc="k", lw=1, fill=True, zorder=400
    )
...
axes[0, :].format(xlabel="Pupil position (m)")
axes[:, 1:].format(yloc="none")
...
axes[1, :].format(xlim=(-8.5, 8.5), ylim=(-8.5, 8.5))
