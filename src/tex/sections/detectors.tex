...
\end{equation}
From this fit we extract the gain ($k$) in \si{\electron/adu} from the slope of the line. We measure the readout noise from the standard deviation of a bias frame and convert to \si{\electron} with the gain. We observe that the full well (\SI{7000}{\electron}) is greater than can be stored in a 16-bit number with our measured detector gain. The detector photoresponse is linear over the entire dynamic range and saturates at $2^{16}$ \si{adu}.

We measured the dark current of the detectors during commissioning with very long exposures while the detector lens caps were covered. We took sequences of two images per detector with increasing exposure times from \SIrange{1}{600}{\second}. We used a bias frame for background subtraction and the detector gain fit above to convert our pixel values to flux in \si{\electron}. We performed a linear fit to this signal with exposure time, only using frames with obvious dark signal (DIT$>$\SI{10}{\second}). For both cameras the dark current at \SI{-40}{\celsius} is \SI{3.6}{\electron/\pixel/\kilo\second}, better than the manufacturer's specification. We report the detector characteristics in \autoref{tbl:detectors}.

\subsection{Photon number resolving}
