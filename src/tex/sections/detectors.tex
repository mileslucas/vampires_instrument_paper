...
\begin{figure}
    ...
    \caption{(top) histogram of pixel values in a 100 by 100 pixel region of a long exposure in ``slow'' mode- all signal is due to dark current. The photon number peaks are clearly resolved. (bottom) the relative S/N improvement from calculating the photon number in the low-flux regime. Solid curves are theoretical values and the scatter points are Monte Carlo statistical simulations shown for both the ``slow'' and the ``fast'' detector readout modes.\label{fig:pnr}}
\end{figure}
