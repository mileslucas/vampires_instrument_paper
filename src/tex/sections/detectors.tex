...
\tablehead{
    \multirow{2}{*}{\colhead{Object}} &
    \colhead{$t_\mathrm{obs}$} &
    \colhead{$\rho$} &
    \colhead{$\theta$} &
    \colhead{$\Delta m$} &
    \multirow{2}{*}{\colhead{Ref.}} \\
    &
    \colhead{(MJD)} &
    \colhead{(mas)} &
    \colhead{(\textdegree)} &
...
\tablecaption{Visual binaries used for astrometric calibration. The ephemeris (separation and position angle) are reported for the average observation time of the data.\label{tbl:binaries}}
\startdata
