...
\end{equation}
where $\mu$ is the average intensity in \si{adu}, $k$ is the gain in \si{\electron/adu}, and $\sigma_{RN}$ is the root-mean-square (RMS) read noise in \si{adu}.
...
The relative sensitivity of the qCMOS detectors compared to the previous EMCCDs is shown in \autoref{fig:detector_snr_relative}. The number of detected photoelectrons is
\begin{equation}
