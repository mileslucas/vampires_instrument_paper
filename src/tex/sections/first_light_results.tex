...
\begin{figure*}[t]
    ...
    \caption{\formatdate{7}{7}{2023} VAMPIRES observations of \textit{HD 169142} in multiband imaging mode. The data from each filter has been summed together. (left) the Stokes $Q_\phi$ image in linear scale. (right) Stokes $Q_\phi\times r^2$, where $r$ is the distance from the image center in linear scale, which better shows the inner edge of the ring at \ang{;;0.6}. All data are rotated so that North is up and East is to the left.\label{fig:hd169142_mosaic}}
\end{figure*}
