...
We reduced this data as described in \autoref{sec:processing} with background subtraction and double-difference polarimetric reduction. We adapt the Mueller matrix coefficients from \citet{zhang_characterizing_2023} using the 625-50 coefficients for the F610 field, 675-50 for F670, 725-50 for F720, and 750-50 for F760. We only use the coefficients for the common-path HWP, image rotator, and generic optics term (i.e., excluding the detector intensity ratio and the FLC, since these changed during upgrades). 

We correct the residual instrumental polarization using the photometric sums in an annulus from \ang{;;0.24} to \ang{;;0.33}, which matches the cavity of the disk \citep{bertrang_hd_2018}. We convert the data from \si{adu} to \si{mJy/arcsec^2} using a synthetic photometry of a standard F2V model spectrum \citep{pickles_stellar_1998} normalized to i=\SI{7.978}{mag} \citep{zacharias_fourth_2013}. We note that our Mueller-matrix correction is not optimal, but future analysis will be done when the upgraded instrument polarization calibration is completed.

The polarized intensity is approximately equal to the azimuthal Stokes $Q_\phi$ frames, which we show summed together in \autoref{fig:hd169142_mosaic}. The inner disk is is obvious, with some diminished intensity to the southwest. We also show a version scaled by the squared stellocentric distance to normalize the stellar irradiation. The scaled version shows the large disk gap and the inner rim of the outer disk. We note that compared to images from \cite{bertrang_hd_2018} we have worse S/N and do not fully resolve the outer disk. However, our observations have smaller spectral bandwidth per image and use a fraction of the detector integration time (\SI{1}{s} versus \SIrange{6}{10}{s}). Future observations with longer detector integration times are needed to resolve the full disk structure.
