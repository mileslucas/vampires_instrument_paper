...
We reduced this data as described in \autoref{sec:processing} with background subtraction and double-difference polarimetric reduction. We adapt the Mueller matrix coefficients from \citet{zhang_characterizing_2023} using the \texttt{625-50} coefficients for the \texttt{F610} field, \texttt{675-50} for \texttt{F670}, \texttt{725-50} for \texttt{F720}, and \texttt{750-50} for \texttt{F760}. We only use the coefficients for the common-path HWP, image rotator, and generic optics term (i.e., excluding the detector intensity ratio and the FLC, since these changed during upgrades). 
...
Sun-observer phase angle & \ang{1.8173}
\enddata
