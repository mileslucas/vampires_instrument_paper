...
\tablehead{\colhead{Name} & \colhead{Type} & \colhead{$D_{in}$ (\si{mm})} & \colhead{$D_{out}$ (\si{mm})} & \colhead{through (\%)}}
...
\enddata
\tablenotes{The stop type is either ``G'' for the gold-plated glass window and ``M'' for the laser-cut metal sheet foil.}
\end{deluxetable}
